...
- `PROBE_SSTI` - SSTI probing
- `PROBE_SSRF` - SSRF probing (cloud metadata style payloads)
- `PROBE_CMDI` - command injection probing
- `PROBE_NOSQLI` - NoSQL injection probing
- `PROBE_XXE` - XXE probing
- `CHECK_SENSITIVE_FILES` - dedicated sensitive file checks
- `CHECK_BACKUP_FILES` - backup/old/temp file variant checks
- `CHECK_COOKIE_SECURITY` - Secure/HttpOnly/SameSite checks
- `CHECK_TLS` - TLS protocol/cipher/certificate checks
- `CHECK_SECURITY_TXT` - `/.well-known/security.txt` discovery
- `CHECK_CORS_PREFLIGHT` - CORS preflight abuse checks
- `CHECK_CACHE_CONTROL` - cacheability checks on sensitive endpoints
- `CHECK_WAF` - WAF detection heuristics
- `CHECK_RESPONSE_FUZZING` - malformed request fuzzing/error leakage check
- `CHECK_OPEN_REDIRECT` - open redirect parameter checks
- `CHECK_SESSION_FIXATION` - heuristic login session fixation check
- `CHECK_CSRF` - heuristic CSRF enforcement check
- `PASSIVE_SUBDOMAIN_ENUM` - passive subdomain discovery via crt.sh
- `WAYBACK_ENUM` - historical endpoint discovery via Wayback CDX
...
- SSTI, SSRF, command injection, NoSQLi and XXE probes (potential findings)
- Sensitive files + backup/old file exposure checks
- Cookie security, TLS posture, cache-control and CORS preflight checks
- WAF detection and malformed-input response fuzzing
- Open redirect and heuristic session fixation/CSRF checks
- Optional passive subdomain + Wayback endpoint enrichment
